...
func nastav_hru():
	...
	kamera.set_limit(SIDE_LEFT, 0)
	kamera.set_limit(SIDE_TOP, 0)
	kamera.set_limit(SIDE_BOTTOM, map_size.y)
	kamera.set_limit(SIDE_RIGHT, map_size.x)
	hrac.speed = speed
...
func vytvor_kamen():
	match randi() % 3:
		0 :
			return kamen_1.instantiate()
		1 :
			return kamen_2.instantiate()
		2 :
			return kamen_3.instantiate()
